...
fn negate(arg: ObjectRef) -> Result<ObjectRef, RuntimeError> {
    if let Some(value) = arg.value::<f64>() {
        Ok(ObjectRef::new_float(-value))
    } else {
        Err(RuntimeError::InvalidType(TY_FLOAT, arg.instance_type_data().type_tag().clone()))
    }
}

fn mul(lhs: ObjectRef, rhs: ObjectRef) -> Result<ObjectRef, RuntimeError> {
    let args = (lhs.value::<f64>(), rhs.value::<f64>());

    match args {
        (Some(lhs), Some(rhs)) => Ok(ObjectRef::new_float(lhs * rhs)),
        (Some(_), None) => Err(RuntimeError::InvalidType(TY_FLOAT, rhs.instance_type_data().type_tag().clone())),
        _ => unreachable!(),
    }
}
...
mod test {
    ...
    #[test]
    fn test_add_with_two_floats() -> Result<(), RuntimeError> {
        let lhs = ObjectRef::new_float(40.0);
        let rhs = ObjectRef::new_float(2.0);
    ...
    #[test]
    fn test_add_with_lhs_float_rhs_none() -> Result<(), RuntimeError> {
        let lhs = ObjectRef::new_float(40.0);
    ...
    #[test]
    fn test_eq_with_two_floats() -> Result<(), RuntimeError> {
        let lhs = ObjectRef::new_float(40.0);
        let rhs = ObjectRef::new_float(2.0);
